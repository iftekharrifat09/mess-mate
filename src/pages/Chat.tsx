...
export default function Chat() {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const result = await api.sendChatMessageAPI(newMessage.trim());
      if (result.success) {
        setNewMessage('');
        await loadMessages();
        setTimeout(scrollToBottom, 150);
      } else {
        toast({ title: 'Failed to send', description: (result as any).error || 'Try again', variant: 'destructive' });
      }
    } catch {
      ...
      <div className="flex flex-col h-[calc(100vh-2rem)] max-h-[calc(100vh-2rem)]">
        ...
        <div className="flex items-center gap-3 pb-4 border-b border-border mb-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{messages.length} messages • {new Set(messages.map(m => m.userId)).size} participants</p>
          </div>
        </div>

        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto pr-1 space-y-1 py-2" ref={scrollRef}>
          {loading ? (
        ...
        <div className="border-t border-border pt-3 mt-2">
          ...
          <p className="text-[10px] text-muted-foreground mt-1 text-right">{newMessage.length}/2000</p>
        </div>
      </div>
    </DashboardLayout>
  );
